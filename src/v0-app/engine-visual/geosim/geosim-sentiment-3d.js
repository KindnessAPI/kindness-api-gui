...
export let simulatePosition = glsl`
  /*
    LIBRARY
  */
  #include <common>
  ...
  void main (void) {
    ...
    */
    ...
    if (!isInvalid) {
      ...
      float dX = (squareIDX / dimension);
      float dY = (mod(squareIDX, dimension));
      float gapper = 3.0;
      ...
      pos.z += sin(time  + pZ * piz * 0.333 + pX * piz * 0.333) * 150.0;
...
export const makeAPI = ({ renderer, scene }) => {
  ...
  posVar.material.defines.DIMENSION3 = `${Math.pow(WIDTH * WIDTH, 1 / 3).toFixed(5.0)};`
  posVar.material.defines.DIMENSION2 = `${Math.pow(WIDTH * WIDTH, 1 / 2).toFixed(5.0)};`

  // state var
  colorVar.material.uniforms.time = { value: 0.0 }
  colorVar.material.uniforms.meta0 = { value: meta0 }
  colorVar.material.defines.DIMENSION3 = `${Math.pow(WIDTH * WIDTH, 1 / 3).toFixed(5.0)};`
  colorVar.material.defines.DIMENSION2 = `${Math.pow(WIDTH * WIDTH, 1 / 2).toFixed(5.0)};`
