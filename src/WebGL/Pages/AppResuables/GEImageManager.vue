<template>
  <div class="p-3 max-w-4xl mx-auto">
    ...
    <table>
      <tr>
        <th class="p-2 text-center"><button class="m-2 py-2 px-3 bg-teal-500 hover:bg-teal-400 rounded-lg text-white" @click="selAll(files)">Select All</button></th>
        <th v-if="pick">Pick</th>
        <th class="p-2 text-center">Preview</th>
        <th class="p-2 text-center">Small</th>
        <th class="p-2 text-center">Optimised Img</th>
        <th class="p-2 text-center">Ext</th>
        <th class="p-2 text-center">Mime</th>
      </tr>
      <tr>
        <td v-if="loading" :colspan="pick ? 7 : 6" class="text-center">Loading</td>
      </tr>
      <tr :key="file._id" v-for="file in files">
        <td class="m-1 flex justify-center py-2">
        ...
        <td class="m-1">
          <a :href="file.cloudinary.secure_url" target="_blank">
            <img class="w-12 h-12 m-1 object-cover object-center" :src="file.cloudinary.secure_url.replace('/upload/', '/upload/w_256,h_256,c_fill,g_auto:0,q_auto/')" alt="">
          </a>
        </td>
        <td class="m-1"><textarea @click="$event.target.select()" :value="file.cloudinary.secure_url.replace('/upload/', '/upload/q_auto/h_200/')" rows="1" cols="10" class="p-2 border-gray-800 border m-1 resize-none"></textarea></td>
        <td class="m-1"><textarea @click="$event.target.select()" :value="file.cloudinary.secure_url.replace('/upload/', '/upload/q_auto/')" rows="1" cols="10" class="p-2 border-gray-800 border m-1 resize-none"></textarea></td>
        <td class="m-1">{{file.ext}}</td>
        <td class="m-1">{{file.mime}}</td>
      </tr>
    </table>
    <!-- <pre>{{ files }}</pre> -->
  </div>
