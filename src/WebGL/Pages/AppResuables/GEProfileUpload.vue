<template>
  <div>
    ...
    <ReButton :color="'teal'" @click="upload">
      ...
      <span v-if="state === 'uploading'">Uploading {{ progress.toFixed(0) }}%</span>
    </ReButton>
  </div>
</template>
    ...
    upload () {
      ...
      filer.onchange = async (evt) => {
        ...
        if (file) {
          ...
          try {
            // let compressed = await loadImage(file, { width: 1024, height: 1024, orientation: true, cover: true })
            let blobURL = URL.createObjectURL(file)
            let srcURL = await MyFiles.snapshotResize(blobURL, 1024, 1024)
